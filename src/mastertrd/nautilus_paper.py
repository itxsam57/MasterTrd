...
def probe_nautilus_sandbox_session(candidate: StrategyGenome) -> PaperStartReceipt:
    ...
    session_id = hashlib.sha256(
        f"{candidate.strategy_id}:{candidate.genome_hash}:{venue}:{engine_version}".encode()
    ).hexdigest()[:24]

...
def test_paper_status_workflow_is_read_only_and_publishes_safe_artifact():
    ...
    assert "src/mastertrd/oracle_paper_status.py" in workflow
    ...
    assert "mastertrd-paper@" in workflow
    assert "journalctl" in workflow
    assert "ExecMainCode" in workflow
    assert "ExecMainStatus" in workflow
    assert "PAPER_STATUS_DIAGNOSTICS_DIR" in workflow
    ...
    forbidden = (
        ...
        "sed -i",
        "tee -a /etc/mastertrd",
        "journalctl -u mastertrd.service",
    )
